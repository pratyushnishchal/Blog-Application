...
public class BlogDao {
	...
	public static void incrementLike(int blogId,int userId) {
		try {
			s=sf.openSession();
			Transaction t=s.beginTransaction();
			Query<Likes> q=s.createQuery("FROM Likes where userid=:userid AND blogid=:blogid",Likes.class);
			q.setParameter("userid",userId);
			q.setParameter("blogid",blogId);
			Likes checkLike=q.uniqueResult();
			if(checkLike==null) {
				Blog b=s.get(Blog.class, blogId);
				b.setLikes(b.getLikes()+1);
				s.update(b);
				Likes newLike=new Likes(blogId,userId);
				s.save(newLike);
			}
			t.commit();			
		}
